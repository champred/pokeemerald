...
{
    MOVE_GROWL, MOVE_ROAR, MOVE_SING, MOVE_SUPERSONIC, MOVE_SCREECH, MOVE_SNORE, MOVE_UPROAR, MOVE_METAL_SOUND, MOVE_GRASS_WHISTLE,
    MOVE_HYPER_VOICE, MOVE_BUG_BUZZ, MOVE_CHATTER, MOVE_RELIC_SONG, MOVE_SNARL, SOUND_MOVES_END
};
...
static u8 ItemEffectMoveEnd(u32 battlerId, u16 holdEffect)
{
	...
	switch (holdEffect)
	{
		...
		case HOLD_EFFECT_RESTORE_HP:
			...
			{
				gBattleMoveDamage = ItemId_GetHoldEffectParam(gLastUsedItem);
				if (gBattleMons[battlerId].hp + gBattleMoveDamage > gBattleMons[battlerId].maxHP)
					gBattleMoveDamage = gBattleMons[battlerId].maxHP - gBattleMons[battlerId].hp;
				gBattleMoveDamage *= -1;
				BattleScriptExecute(BattleScript_ItemHealHP_RemoveItem);
