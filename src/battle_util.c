...
#define CHECK_ABILITY(a) for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++){\
            if (side != GetBattlerSide(gActiveBattler)\
                && *(gBattleStruct->moveTarget + gBattlerAttacker) != gActiveBattler\
                && gBattleMons[gActiveBattler].ability == a\
...
void HandleAction_UseMove(void)
{
                 ...
                 || gBattleMoves[gCurrentMove].target != MOVE_TARGET_USER))
    {
		...
		CHECK_ABILITY(ABILITY_LIGHTNING_ROD)
		...
		CHECK_ABILITY(ABILITY_STORM_DRAIN)
	}
...
void HandleAction_Run(void)
{
    ...
    else
    {
        ...
        {
            if (gBattleMons[gBattlerAttacker].status2 & (STATUS2_WRAPPED | STATUS2_ESCAPE_PREVENTION)&&!IS_BATTLER_OF_TYPE(gBattlerAttacker,TYPE_GHOST))
            {
...
static const u16 sSoundMovesTable[] =
{
    ...
    MOVE_BUG_BUZZ, MOVE_CHATTER, MOVE_RELIC_SONG, MOVE_SNARL, SOUND_MOVES_END
};
